...
@extends('admin.file-manager.layout')
                            ...
                            <div  class="tab-pane @if($loop->first) active @endif" id="tab_{{$cropName}}_{{$size}}" style="position:relative;overflow: auto">
                                ...
                                @else
                                    ...
                                    <div class="col-md-4" style="direction: ltr">
                                        ...
                                        <div>
                                            <button onclick="$('input[data-cropkey={{$size}}]').click();$('#demo8_form').submit();" class="btn green-haze btn">{{trans('admin.file-manager.crop.buttons.save')}}</button>
    ...
    function ajaxSend()
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                showToast('{{trans('admin.file-manager.crop.messages.success.message')}}', '{{trans('admin.file-manager.crop.messages.success.title')}}', 'success');
                App.unblockUI();
            },
            error: function() {
              showToast('{{trans('admin.file-manager.crop.messages.error.message')}}', '{{trans('admin.file-manager.crop.messages.error.title')}}', 'error');
                App.unblockUI();
            }
        });
